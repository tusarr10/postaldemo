...
Public Class sbaclist
    ...
    Private Sub sbaclist_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        DataGridView2.GridColor = Color.Pink
        DataGridView1.GridColor = Color.Red
        DataGridView2.BackgroundColor = Color.OrangeRed
        DataGridView1.BackgroundColor = Color.Blue
        DataGridView2.ForeColor = Color.DarkGreen
    ...
    Sub filt()
        Try
            ...
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT AcNo as [Account Number],CustmorName as [Account Holder Name],Acbal as [Account Balance],DLT,DLT2,AcType as [Account Type] from sbbal where AcNo like '%" & SBAc.account_no.Text & "%' and AcType = 'Saving Account' order by CustmorName", con)
            Dim myDA1 As OleDbDataAdapter = New OleDbDataAdapter(cmd)
            Dim myDataSet1 As DataSet = New DataSet()
            myDA1.Fill(myDataSet1, "sbbal")
            DataGridView2.DataSource = myDataSet1.Tables("sbbal").DefaultView
    ...
    Private Sub DataGridView1_RowHeaderMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DataGridView1.RowHeaderMouseClick
        ...
        Try
            ...
            '  Me.Hide()
            '  SBAc.Hide()
            If Len(Trim(SBAc.account_no.Text)) = 0 Then
                SBAc.account_no.Text = dr.Cells(0).Value.ToString()
            ...
            'SBAc.Accountbal.Text = dr1.Cells(2).Value.ToString()
            'SBAc.DLT.Text = dr1.Cells(3).Value.ToString()
            'SBAc.DNT.Text = dr1.Cells(4).Value.ToString()
            '*******************************For verification*********************************
            ...
            Try
                con = New OleDbConnection(cs)
                con.Open()
                cmd = New OleDbCommand("SELECT AcNo as [Account Number],CustmorName as [Account Holder Name],Acbal as [Account Balance],DLT,DLT2,AcType as [Account Type] from sbbal where AcNo = '" & SBAc.account_no.Text & "' order by CustmorName", con)
                Dim myDA1 As OleDbDataAdapter = New OleDbDataAdapter(cmd)
                Dim myDataSet1 As DataSet = New DataSet()
                myDA1.Fill(myDataSet1, "sbbal")
                DataGridView2.DataSource = myDataSet1.Tables("sbbal").DefaultView
