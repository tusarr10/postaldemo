...
Public Class Rdlist
    ...
    Sub filt()
        Try
            ...
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT AcNo as [Account Number],CustmorName as [Account Holder Name],Acbal as [Account Balance],DLT,DLT2,AcType as [Account Type] from sbbal where AcNo like '%" & RdAc.account_no.Text & "%' and AcType = 'Requiring Deposit' order by CustmorName", con)
            Dim myDA1 As OleDbDataAdapter = New OleDbDataAdapter(cmd)
            Dim myDataSet1 As DataSet = New DataSet()
            myDA1.Fill(myDataSet1, "sbbal")
            DataGridView2.DataSource = myDataSet1.Tables("sbbal").DefaultView
    ...
    Private Sub DataGridView1_RowHeaderMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DataGridView1.RowHeaderMouseClick
        ...
        Try
            ...
            '  Me.Hide()
            '  rdac.Hide()
            If Len(Trim(RdAc.account_no.Text)) = 0 Then
                RdAc.account_no.Text = dr.Cells(0).Value.ToString()
            ...
            Try
                con = New OleDbConnection(cs)
                con.Open()
                cmd = New OleDbCommand("SELECT AcNo as [Account Number],CustmorName as [Account Holder Name],Acbal as [Account Balance],DLT,DLT2,AcType as [Account Type] from sbbal where AcNo = '" & RdAc.account_no.Text & "' order by CustmorName", con)
                Dim myDA1 As OleDbDataAdapter = New OleDbDataAdapter(cmd)
                Dim myDataSet1 As DataSet = New DataSet()
                myDA1.Fill(myDataSet1, "sbbal")
                DataGridView2.DataSource = myDataSet1.Tables("sbbal").DefaultView
    ...
    Private Sub Rdlist_Load(sender As Object, e As EventArgs) Handles MyBase.Load
